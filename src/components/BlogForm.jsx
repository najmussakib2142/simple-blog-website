...
export default function BlogForm() {
  ...
  // ✅ Submit Blog (SECURE + STABLE)
  const handleSubmit = async (e) => {
    e.preventDefault();

    if (!user?.uid) {
      Swal.fire("Login required", "You must be logged in.", "error");
      return;
    }
    ...
    if (
      ...
      Swal.fire("Missing Fields", "All required fields must be filled.", "error");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (data?.error?.toLowerCase().includes("slug")) {
          Swal.fire("Duplicate Title", "Try a different blog title.", "warning");
        } else {
          Swal.fire("Error", data?.error || "Failed to create blog", "error");
        }
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      {/* ✅ YOUR UI BELOW IS 100% UNCHANGED ✅ */}
      ...
      <div className="flex-col md:flex gap-10 md:justify-between md:flex-row md:items-center">
        {/* Category */} <div className="flex-1"> <label className="block text-sm font-semibold text-gray-900 mb-2">Category</label>
          <select name="category" value={formData.category} onChange={handleInputChange} required className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black/40 focus:border-transparent bg-white text-gray-900">
            <option className="" value="">Select category</option> <option value="Tech">Tech</option> <option value="Lifestyle">Lifestyle</option> <option value="Business">Business</option> <option value="Health">Health</option>
      ...
      <div>
        ...
        <input type="text" name="tags" value={formData.tags.join(", ")} onChange={handleInputChange} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-black/40 focus:border-transparent bg-white text-gray-900 placeholder-gray-500" placeholder="React, Next.js, Cloudinary" />
      </div>
      {/* Featured */}
      {user?.role === "admin" && (<div className="flex text-black items-center gap-2"> <input type="checkbox" name="featured" checked={formData.featured} onChange={handleInputChange} /> <label>Mark as Featured</label> </div>)}
      {/* Image Upload */}
      <div className="w-full">
        ...
        {/* Loader */}
        {imageUploading && (<div className="mt-4 p-4 bg-white rounded-xl border border-gray-200 shadow-sm flex items-center gap-3">
          <div className="border-4 border-gray-300 border-t-black/40 rounded-full w-8 h-8 animate-spin">
          </div>
          <p className="text-sm text-gray-600">Uploading image…</p>
        </div>)}
        {/* Preview */}
        {!imageUploading && formData.imageUrl && (<div className="mt-4 p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
      ...
      <div className="flex gap-3 pt-4">
        <button type="submit" disabled={loading || imageUploading}
          className={`flex-1 py-3 rounded-lg font-medium transition-colors ${loading ? "bg-gray-400 text-white cursor-not-allowed" : "bg-black/90 text-white hover:bg-black"}`}>
